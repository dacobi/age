...
func apply_wheel_physics(car: RigidBody3D) -> void:
	...
	if visual_wheel and is_instance_valid(visual_wheel):
		...
		if visual_wheel is CSGCylinder3D:
			axis = Vector3(0, 1, 0) # CSGCylinders align along Y axis
		visual_wheel.rotate_object_local(axis, visual_vel * get_physics_process_delta_time() / wheel_radius)
