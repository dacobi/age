...
static func _build_transition(root: Node3D, length: float, sw: float, ew: float, incline: float = 0.0):
	if abs(incline) < 0.1:
		var road = CSGPolygon3D.new()
		road.mode = CSGPolygon3D.MODE_DEPTH
		road.depth = 0.5 
		...
		for i in range(num_slices):
			var t1 = float(i) / num_slices
			var t2 = float(i + 1) / num_slices
			
			var angle1 = t1 * abs(theta)
			var angle2 = t2 * abs(theta)
			
			var y1 = (R - R * cos(angle1)) * sign_pitch
			var z1 = -R * sin(angle1)
			
			var y2 = (R - R * cos(angle2)) * sign_pitch
			var z2 = -R * sin(angle2)
			
			var slice_len = Vector3(0, y1, z1).distance_to(Vector3(0, y2, z2))
			var w1 = lerp(sw, ew, t1)
			var w2 = lerp(sw, ew, t2)
			
			var slice_node = Node3D.new()
			slice_node.position = Vector3(0, y1, z1)
			slice_node.rotation.x = angle1 * sign_pitch
			root.add_child(slice_node)
			
			var road = CSGPolygon3D.new()
			road.mode = CSGPolygon3D.MODE_DEPTH
			road.depth = 0.5 
			road.polygon = PackedVector2Array([
				Vector2(-w1/2.0, 0), Vector2(-w2/2.0, slice_len), 
				Vector2(w2/2.0, slice_len), Vector2(w1/2.0, 0)
			])
			road.rotation_degrees.x = -90
			road.use_collision = true
			road.material = get_road_mat()
			slice_node.add_child(road)
			
			var c_line = CSGPolygon3D.new()
			c_line.mode = CSGPolygon3D.MODE_DEPTH
			c_line.depth = 0.1
			c_line.polygon = PackedVector2Array([
				Vector2(-1.2, 0), Vector2(-1.2, slice_len), 
				Vector2(1.2, slice_len), Vector2(1.2, 0)
			])
			c_line.rotation_degrees.x = -90
			c_line.position.y = 0.25
			c_line.material = get_centerline_mat()
			slice_node.add_child(c_line)
			...
			border_l.depth = 4.0
			border_l.polygon = PackedVector2Array([
				Vector2(-w1/2.0 - w/2.0, 0), Vector2(-w2/2.0 - w/2.0, slice_len), 
				Vector2(-w2/2.0 + w/2.0, slice_len), Vector2(-w1/2.0 + w/2.0, 0)
			])
			border_l.rotation_degrees.x = -90
			border_l.position.y = 4.0
			border_l.use_collision = true
			border_l.material = get_cyan_mat()
			slice_node.add_child(border_l)
...
static func generate(track_data: Array, root_node: Node3D):
	...
	for i in range(track_data.size()):
		...
		if piece["type"] == "straight":
			...
			var incline = piece.get("incline", 0.0)
			_build_transition(root, length, sw, ew, incline)
			if abs(incline) < 0.1:
				end_transform = current_transform.translated_local(Vector3(0, 0, -length))
			else:
				var theta = deg_to_rad(incline)
