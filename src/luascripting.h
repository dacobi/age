...
struct LuaSyncData {
    ...
    uint64_t object_id_res = 0;
    uint64_t object_id_arg = 0;
    int res_type = 0;
};
...
using PlayPlaylistTrackFunc = std::function<void(int)>;
    ...
    void setMouseMotion(float dx, float dy);
...
private:
    ...
    std::map<std::pair<uint64_t, std::string>, float> last_float_sets;
    std::map<std::pair<uint64_t, std::string>, std::string> last_string_sets;
    std::map<std::pair<uint64_t, std::string>, bool> last_bool_sets;
...
private:
    ...
    static int lua_imguiRemoveWindow(lua_State* L);
    ...
    float mouse_dx = 0.0f;
    float mouse_dy = 0.0f;
    std::unordered_map<std::string, bool> kb_click_state;
    std::unordered_map<int, bool> joy_button_click_state;
    std::unordered_map<int, bool> mouse_click_state;
