...
def make_march():
    """The iconic heavy thumping sound of the moving aliens."""
    duration = 0.12 # Short thump
    num_samples = int(SAMPLE_RATE * duration)
    data = []
    phase = 0.0
    ...
    for i in range(num_samples):
        t = i / SAMPLE_RATE
        # Pitch drops very quickly from 120Hz to 60Hz
        freq = 120 - (60 * (t / duration))
        
        phase += (2 * math.pi * freq) / SAMPLE_RATE
        # Triangle-like waveform for a punchier bass thump
        sample = 0.4 * (abs((phase % (2 * math.pi)) - math.pi) / math.pi - 0.5)
